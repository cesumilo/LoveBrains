...
// Last update Sat Aug  1 14:49:47 2015 Guillaume ROBIN
//
...
bool		ColliderAIAndFood::isCollision(Graphics::IObject *obj_1, Graphics::IObject *obj_2)
{
  ...
  if (obj_1->getType().compare("basic_survival_ai") && obj_2->getType().compare("basic_food"))
    {
      ai = (BasicAI *)obj_1;
      food = (BasicFood *)obj_2;
    }
  else if (obj_1->getType().compare("basic_food") && obj_2->getType().compare("basic_survival_ai"))
    {
      ai = (BasicAI *)obj_2;
